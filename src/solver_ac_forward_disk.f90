...
subroutine solver_ac_forward_disk(rank,NumSou,NumRec,nxSou,nySou,nxRec,nyRec,&
S,dmodel,dt,ny,nx,ctmp,&
shotid,unit_fields,folder_fields,&
store_snap,nt_snap,nt,Rec_point)
...
implicit none
	...
	allocate(Fields(ny,nx))

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        
!        write(500,*)rank,iprop,NumSou,NumRec,d,dt,ny,nx,store_snap,nt_snap,nt        
!        do i=1,NumSou
!                write(501,*)i,nxSou(i),nySou(i)	!!ojo nxSou es diferente
!        enddo
        
!        do i=1,NumRec
!                write(502,*)i,nxRec(i),nyRec(i)!!nxRec es diferente
!        enddo
...
!!! 		Field and Traces saving
		if(iprop.eq.1)	then
			...
			snap=snap+1
			if(snap.gt.nt_snap)write(*,*)'nap overflow 1',snap

	                write(Str_snap,*) snap
			file_fields_snap= trim(adjustl(folder_fields)) // "fields_shot_"// trim(adjustl(Str_shot)) // &
			"_snap_" // trim(adjustl(Str_snap)) // ".bin"

			!write(*,*)"name: ", trim(adjustl(file_fields_snap))

			open(unit=unit_fields,file=file_fields_snap,form='unformatted',&
			access='stream', status='unknown', action='write', iostat=ierr)

			if (ierr /= 0) stop 'Error opening file for writing snapshots'

			do i=1,nx
			do j=1,ny
				Fields(j,i)=(p_3(dny2+j,dnx+i)-p_1(dny2+j,dnx+i))*0.5/dt
			enddo
			enddo

			write(unit_fields)Fields
...
!	if(iprop.eq.1)	then
!		open(unit=12,file="forward.dat",status='unknown')
!	        do j=1,nt
