subroutine solver_ac_forward_ram_tt(rank,NumSou,NumRec,nxSou,nySou,nxRec,nyRec,&
S,dmodel,dt,ny,nx,ctmp,&
store_snap,nt_snap,nt,Rec_point,TT_s)
...
!	if(iprop.eq.1)	then
!		write(*,*)"nxt,nyt",nxt,nyt
!		write(*,*)"nxs,nys",nxs,nys
!	endif
...
!	do j=1,ny
!		write(100,'(20000(e12.5,2x))') (ctmp(j,i),i=1,nx)
!	enddo
	
!	do j=1,nyt
!		write(101,'(20000(e12.5,2x))') (c(j,i),i=1,nxt)
!	enddo
...
!!!		Field and Traces saving
		if(iprop.eq.1)	then
			...
			call TT_source_onfly(p_3,p_1,TT_s,amp_prev,snap,dt,dnx,dny,nx,ny)

			if(snap.gt.nt_snap)write(*,*)'nap overflow 1',snap

!			do i=1,nx
!			do j=1,ny
!				Fields(j,i,snap)=(p_3(dny2+j,dnx+i)-p_1(dny2+j,dnx+i))*0.5/dt
!			enddo
!			enddo
	...
	do i=1,NumRec
		Rec_point(nt,i)=Rec_point(nt-1,i)
	enddo
