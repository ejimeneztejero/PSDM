subroutine calculate_PSDM_eikonal(Vpmodel,Batmodel,new_comm,PSDM_depth)
...
real :: theta,cos_theta,pxs,pzs,pxr,pzr
...
if(rank.eq.0)write(*,*) "Allocating Eikonal dimensions"
...
if(icount.ge.line_i.and.icount.le.line_f)  then
	...
	if(rank.eq.0)write(*,*) "Dynamic window in X (ptos) (nx_w,x_i,x_f): ",nx_w,x_i,x_f
        ...
        if(rank.eq.0)write(*,*)
        if(rank.eq.0)write(*,*)"Eikonal equation ..."
...
!!	Imaging condition:
	...
	do k=1,NumRec
	   do i=1,nx_w
		...
		!weight = 1.0 / sqrt( max(tt_tot, dt_prop) )

		if(theta_max.le.0.) then

			PSDM_w(j,i) = PSDM_w(j,i) + Data_target(it,k)	

		else 

			! Gradientes de T (dirección del rayo)
			if(i.lt.nx_w.and.i.gt.1) pxs = (TT_s(j,i+1) - TT_s(j,i-1)) / (2*dmodel_prop)
			if(j.lt.ny_prop.and.j.gt.1) pzs = (TT_s(j+1,i) - TT_s(j-1,i)) / (2*dmodel_prop)
			if(i.lt.nx_w.and.i.gt.1) pxr = (TT_r(j,i+1,k) - TT_r(j,i-1,k)) / (2*dmodel_prop)
			if(j.lt.ny_prop.and.j.gt.1) pzr = (TT_r(j+1,i,k) - TT_r(j-1,i,k)) / (2*dmodel_prop)
	
			! Ángulo medio de apertura
			cos_theta = (pxs*pxr + pzs*pzr) / &
			            (sqrt(pxs**2 + pzs**2) * sqrt(pxr**2 + pzr**2))
			theta = acos(cos_theta) * 180.0 / 3.14159
		
			if(theta.le.theta_max) then	! Filtro angular
	...
	deallocate(Vp_w)
        ...
        if(rank.eq.0) then
                ...
                file_name=trim(folder_output) // trim("PSDM_eikonal")

                file_V_twt=trim(folder_output) // trim("Velocity_twt")
                file_P_twt=trim(folder_output) // trim("PSDM_eikonal_twt")
...
end subroutine calculate_PSDM_eikonal
