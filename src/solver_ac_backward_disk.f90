...
subroutine solver_ac_backward_disk(rank,NumSou,NumRec,nxSou,nySou,nxRec,nyRec,&
Sadj,dmodel,dt,ny,nx,ctmp,&
shotid,unit_fields,folder_fields,&
store_snap,nt_snap,nt,kernel)
...
!	write(22,*)"Starts calculation backward: ",nt
...
!!!             FREE SURFACE CALCULATION
		...
		if(iprop.eq.2)	then
			...
			write(22,*)"name: ",file_fields_snap,nt,k

			open(unit=unit_fields, file=file_fields_snap, form='unformatted',&
			access='stream', status='old', action='read', iostat=ierr)

			if (ierr /= 0) stop 'Error opening file for reading snapshots'

			read(unit_fields)Fields

			do i=1,nx
			do j=1,ny
				kernel(j,i)=kernel(j,i)+0.5*(p_3(dny2+j,dnx+i)-p_1(dny2+j,dnx+i))*Fields(j,i)*store_snap
			enddo
			enddo
			...
			close(unit_fields)
		        ...
		        command="rm " // trim(adjustl(file_fields_snap))
            	        call system(command)
